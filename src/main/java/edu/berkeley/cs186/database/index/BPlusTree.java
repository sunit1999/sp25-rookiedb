...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        int index = Collections.binarySearch(leafNode.getKeys(), key);

        if (index < 0) {
            return Optional.empty();
        }

        return Optional.of(leafNode.getRids().get(index));
    }
    ...
    public Iterator<RecordId> scanAll() {
        ...
        return new BPlusTreeIterator(leafNode, leafNode.scanAll());
    }
    ...
    private BPlusNode getNewRoot(DataBox orphanKey, Long orphanNodePageNum) {
        // Create new root with keys and children
        List<DataBox> newRootKeys = new ArrayList<>();
        List<Long> newRootChildren = new ArrayList<>();

        newRootKeys.add(orphanKey);
        newRootChildren.add(root.getPage().getPageNum());
        newRootChildren.add(orphanNodePageNum);

        return new InnerNode(metadata, bufferManager, newRootKeys, newRootChildren, lockContext);
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        LeafNode current;
        Iterator<RecordId> itr;
        BPlusTreeIterator(LeafNode _current, Iterator<RecordId> _itr) {
            current = _current;
            itr = _itr;
        }
        ...
        public boolean hasNext() {
            ...
            if (current != null) {
                ...
                if (current.getRightSibling().isPresent()) {
                    return current.getRightSibling().get().scanAll().hasNext();
                }
            }
